...
#' 
#' @example inst/examples/scale_context.r
...
#'   standard type (passed to \code{\link{scale_standard}}).
...
scale_context <- function(data, var, scale) {
  ...
  if (is.character(scale)) {
    ...
    scale <- scale_standard(x = data[[var]], scale = scale)
...
#' @export
scales_context <- function(data, vars = names(scales), scales) {
  stopifnot(length(vars) == length(scales))
  do.call(cbind, lapply(1:length(scales), function(i) {
    scale_context(data = data, var = vars[i], scale = scales[[i]])
  }))
}
